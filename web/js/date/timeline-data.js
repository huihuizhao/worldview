...
export function timelineData(models, config, ui) {
  ...
  self.end = function() {
    return new Date(new Date(util.now()).setUTCDate(util.now().getUTCDate()));
  };
  ...
  self.set = function() {
    ...
    if (compareModel) {
      let layerGroupString = getActiveLayerGroupString(
        compareModel.active,
        compareModel.isCompareA
      );
      activeLayerGroup = models.layers[layerGroupString];
    }
    var activeLayers = models.layers.get({}, activeLayerGroup);
