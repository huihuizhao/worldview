...
import { palettesTranslate } from '../palettes/util';

export function layersOptions(config, models, layer, layerGroupStr) {
  var $dialog;
  var $opacity;
  var $range;
  var self = {};
  ...
  var renderOpacity = function($dialog) {
    ...
    var $slider = $('<div></div>')
      ...
      .on('slide', function() {
        models.layers.setOpacity(layer.id, $(this).val(), layerGroupStr);
      });
    var $label = $('<div></div>')
      .addClass('wv-label')
  ...
  var rerenderRange = function() {
    var legend = models.palettes.get(layer.id, index, layerGroupStr);
  ...
  var rerenderPaletteSelector = function(firstTime) {
    ...
    $('#wv-palette-selector input').on('ifChecked', function() {
      ...
      setTimeout(function() {
        ...
        if (id === '__default') {
          models.palettes.clearCustom(layer.id, index, layerGroupStr);
        } else {
          models.palettes.setCustom(layer.id, id, index, layerGroupStr);
        }
      }, 0);
    });
  ...
  var customLegend = function(id) {
    var source = models.palettes.getDefaultLegend(
      layer.id,
      index,
      layerGroupStr
    );
    var target = models.palettes.getCustom(id, layerGroupStr);
    var targetType =
